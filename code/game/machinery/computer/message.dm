		...
		if (href_list["select"])
			if(src.linkedServer == null || (src.linkedServer.stat & (NOPOWER|BROKEN)))
				...
				switch(href_list["select"])
					...
					//Select Your Name
					if("Sender")
						customsender 	= sanitize_russian(stripped_input(usr, "Please enter the sender's name."))
					...
					//Enter custom job
					if("RecJob")
						customjob	 	= sanitize_russian(stripped_input(usr, "Please enter the sender's job."))

					//Enter message
					if("Message")
						custommessage	= sanitize_russian(stripped_input(usr, "Please enter your message."))
