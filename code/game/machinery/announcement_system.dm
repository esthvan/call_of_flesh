...
/obj/machinery/announcement_system/proc/CompileText(str, user, rank) //replaces user-given variables with actual thingies.
	str = replacetext510(str, "%PERSON", "[user]")
	str = replacetext510(str, "%RANK", "[rank]")
	return str
...
/obj/machinery/announcement_system/Topic(href, href_list)
	...
	if(href_list["ArrivalTopic"])
		var/NewMessage = sanitize_russian(stripped_input(usr, "Enter in the arrivals announcement configuration.", "Arrivals Announcement Config", arrival))
		if(!in_range(src, usr) && src.loc != usr && !isAI(usr))
			return
		if(NewMessage)
			arrival = NewMessage
	else if(href_list["NewheadTopic"])
		var/NewMessage = sanitize_russian(stripped_input(usr, "Enter in the departmental head announcement configuration.", "Head Departmental Announcement Config", newhead))
		if(!in_range(src, usr) && src.loc != usr && !isAI(usr))
			return
		if(NewMessage)
			newhead = NewMessage
