...
/obj/machinery/door/poddoor/sidor/skadovsk/special/open(ignorepower = 0)
	if(!round_start_time || world.time >= round_start_time + SKADOVSK_LOCKED_TIMER)
		return ..()
	else
		say("Door will be locked for next [round((round_start_time + SKADOVSK_LOCKED_TIMER - world.time)/600) + 1] min. due to psionic emission.")
		return 0