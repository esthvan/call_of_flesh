		...
		if (!(direct & (direct - 1))) //Cardinal move
			...
			if (direct & 1)
				if (direct & 4)
					...
					//if(told.CanPass(src, t))
					if (step(src, NORTH))
						//. = 1//get_step(src, NORTH)
						moving_diagonally = 0
						. = step(src, EAST)
				...
				if (direct & 4)
					moving_diagonally = 2
					//var/turf/t = get_turf(get_step(src, SOUTH))
					//world << "[t.CanPass(src, t)]"
					//if(told.CanPass(src, t))
					if (step(src, SOUTH))
						//. = 1//get_step(src, SOUTH)
						moving_diagonally = 1
						. = step(src, EAST)
					else
						//sleep(movement_delay())
