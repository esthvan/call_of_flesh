...
/mob/living/simple_animal/attackby(obj/item/O, mob/living/user, params) //Marker -Agouri
	...
	if(stat == DEAD)
		if(istype(O, /obj/item/weapon/stalker/knife))
			if(loot.len)
				user.visible_message("<span class='notice'>[user] начал разделывать [src]...</span>", "<span class='notice'>¬ы начали разделывать [src]...</span>")
				if(do_after(user, 30, 1, src))
					user.visible_message("<span class='green'>[user] разделал [src].</span>", "<span class='green'>¬ы разделали [src].</span>")
					if(random_loot)
...
/mob/living/simple_animal/Move(NewLoc, direct)
	//if(world.time < ability_cooldown + previous_time)
	//	return 0
	if(world.time < move_delay && moving_diagonally != 2)
		return 0

	//if(!(moving_diagonally == 2))
	move_delay = movement_delay() + world.time
