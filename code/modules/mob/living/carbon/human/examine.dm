...
/*
	if(back2)
		if(back2.blood_DNA)
			msg += "<span class='warning'>[t_He] [t_has] \icon[back2] [back2.gender==PLURAL?"some":"a"] blood-stained [back2] on [t_his] back.</span>\n"
		else
			msg += "[t_He] [t_has] \icon[back2] \a [back2] on [t_his] back.\n"
*/
	//left hand
	...
	if(!appears_dead)
		...
		if(getorgan(/obj/item/organ/internal/brain))
			if(istype(src,/mob/living/carbon/human/interactive))
				...
				msg += "<span class='deadsay'>[t_He] [t_is] totally catatonic. The stresses of life in the Zone must have been too much for [t_him]. Any recovery is unlikely.</span>\n"
			else if(!client)
