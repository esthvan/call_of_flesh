			...
			if("Yes")
				...
				if(istype(I, /obj/item/device/flashlight))
					...
					if (F.on)
						...
						F.set_light(0)
				/*
				else if(istype(I, /obj/item/device/detector))
					var/obj/item/device/detector/D = I
					if (D.on)
						D.on = 0
						D.user = null
						D.icon_state = initial(icon_state)
						SSobj.processing.Remove(D)
				*/
				else if(istype(I, /obj/item/clothing/head/hardhat))
