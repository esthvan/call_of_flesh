...
/mob/proc/handle_hostile_activity(mob/user, what_done)
	if(stat == DEAD)
		return
	...
	switch(what_done)
		...
		if("punched")
			hostiles[user.ckey] = hostiles[user.ckey] + 3 * multiplier
		if("attacked")
			hostiles[user.ckey] = hostiles[user.ckey] + 10 * multiplier
		if("disarmed")
			hostiles[user.ckey] = hostiles[user.ckey] + 1 * multiplier
	lastattacker = user

/mob/proc/get_killer()
	return lastattacker

/mob/proc/get_biggest_hostile()
	var/biggestHostilePoints = 0
	var/biggestHostile = null
	for(var/mob/user in hostiles)
		if(biggestHostilePoints < hostiles[user.ckey])
			biggestHostile = user
			biggestHostilePoints = hostiles[user.ckey]
	return biggestHostile



	//var/newkarma = GetKarma(killer.last_ckey) - hostiles[killer.ckey]
	//StalkerKarma.SetKarma(killer.last_ckey, newkarma)
...
/mob/verb/abandon_mob()
	...
	if (timeofdeath + config.respawn_timer > world.time)
		if(usr.client.prefs.chat_toggles & CHAT_LANGUAGE)
			usr << "<span class='boldnotice'>You will be able join the game in around [max(1, round((timeofdeath + config.respawn_timer - world.time)/600))] min.</span>"
		else
			usr << "<span class='boldnotice'>¬ы сможете зайти в игру примерно через [max(1, round((timeofdeath + config.respawn_timer - world.time)/600))] мин.</span>"
		return
