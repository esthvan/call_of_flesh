			...
			for(var/i = 0, i < copies, i++)
				if(toner > 0 && !busy && copy)
					...
					if(length(copy.info) > 0)	//Only print and add content if the copied doc has words on it
						...
						copied = replacetext510(copied, "<font face=\"[PEN_FONT]\" color=", "<font face=\"[PEN_FONT]\" nocolor=")	//state of the art techniques in action
						copied = replacetext510(copied, "<font face=\"[CRAYON_FONT]\" color=", "<font face=\"[CRAYON_FONT]\" nocolor=")	//This basically just breaks the existing color tag, which we need to do because the innermost tag takes priority.
						c.info += copied
						c.info += "</font>"
						c.name = copy.name
