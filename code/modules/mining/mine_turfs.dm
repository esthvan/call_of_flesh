...
/turf/simulated/mineral/New()
	...
	spawn(1)
		var/turf/T
		if((istype(get_step(src, NORTH), /turf/simulated/floor)) || (istype(get_step(src, NORTH), /turf/stalker/floor)))
			T = get_step(src, NORTH)
			if (T)
				T.overlays += rockTurfEdgeCache[SOUTH_EDGING]
		if((istype(get_step(src, SOUTH), /turf/simulated/floor)) || (istype(get_step(src, SOUTH), /turf/stalker/floor)))
			T = get_step(src, SOUTH)
			if (T)
				T.overlays += rockTurfEdgeCache[NORTH_EDGING]
		if((istype(get_step(src, EAST), /turf/simulated/floor)) || (istype(get_step(src, EAST), /turf/stalker/floor)))
			T = get_step(src, EAST)
			if (T)
				T.overlays += rockTurfEdgeCache[WEST_EDGING]
		if((istype(get_step(src, WEST), /turf/simulated/floor)) || (istype(get_step(src, WEST), /turf/stalker/floor)))
			T = get_step(src, WEST)
			if (T)
