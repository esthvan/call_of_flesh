...
/datum/subsystem/vote/proc/initiate_vote(vote_type, initiator_key)
	if(!mode)
		...
		switch(vote_type)
			...
			if("custom")
				question = sanitize_russian(stripped_input(usr,"What is the vote for?"))
				if(!question)	return 0
				for(var/i=1,i<=10,i++)
					var/option = capitalize(sanitize_russian(stripped_input(usr,"Please enter an option or hit cancel to finish")))
