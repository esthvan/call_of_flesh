...
/datum/mind/Topic(href, href_list)
	...
	if (href_list["role_edit"])
		...
		if (istype(new_objective, /datum/objective/custom)) //it's lame I know but this type is snowflaky by its very nature
			var/expl = sanitize_russian(stripped_input(usr, "Custom objective:", "Objective", objective ? objective.explanation_text : ""))
			if (!expl)
				expl = "Free objective"
		...
		switch(href_list["monkey"])
			...
			if("infected")
				if (check_rights(R_ADMIN, 0))
					...
					var/mob/living/carbon/monkey/M = current
					if (istype(H))
						log_admin("[key_name(usr)] attempting to  and infect [key_name(current)]")
